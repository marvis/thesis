...
The fast marching (FM) algorithm \cite{sethian1999level}, which is essentially a region-growing scheme, plays an important role in APP2. Both GSDT and initial neuron reconstruction are implemented in the fast marching framework. 
...
\end{algorithm}
\subsection{Min Heap}
Min heap serves as the priority queue in Fast marching method, which enables a fast way to get the minimum element. A min heap is a complete binary tree which contains alternating min and max levels. The root element is always the minimum element in a min-heap. There are two basic operations on a min-heap, that is the inserting an element and deletion the minimum element.
